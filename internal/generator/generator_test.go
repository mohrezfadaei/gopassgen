...
func TestGeneratePassword(t *testing.T) {
	tests := []struct {
		...
		{"Include Specific Symbols", 16, false, false, false, false, "}I<DC", "", false},
		{"Only Include Characters", 16, true, true, true, true, "}I<DC", "", false},
		{"Invalid Length", -1, false, false, false, false, "", "", false},
		{"No Characters Available", 16, true, true, true, true, "", "", true},
		{"Just Include Characters", 16, true, true, true, true, "", "ABC123", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			password, err := GeneratePassword(tt.length, tt.excludeUppercase, tt.excludeLowercase, tt.excludeDigits, tt.excludeSymbols, tt.includeChars, tt.justInclude)
